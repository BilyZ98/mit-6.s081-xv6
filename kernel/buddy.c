...
int pair_bit_isset(char *array, int index) {
...
// flip the bit at position index / 2 
void flip(char *array, int index) {
  char b = array[index / 16];
  char m = (1 << (index % 16)/2);
  array[index/16] = (b ^ m);
}
...
bd_malloc(uint64 nbytes)
{
  ...
  flip(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    flip(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    */
    ...
    flip(bd_sizes[k].alloc, bi);
    if(pair_bit_isset(bd_sizes[k].alloc, buddy)) { // is buddy allocated?
      break;
    }
...
bd_initfree_pair(int k, int bi, int is_left) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(pair_bit_isset(bd_sizes[k].alloc, buddy)) {
    // one fo the pair is free
    free = BLK_SIZE(k); 
    int target = is_left ? bi : buddy;
    if(pair_bit_isset(bd_sizes[k].alloc, target)){
      printf("%s %d:%d | %d %d\n", (is_left?"left":"right"), k, target, bi, buddy);
      lst_push(&bd_sizes[k].free, addr(k, target));
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char)* ROUNDUP(NBLK(k), 16)/16; // use one bit for states of allocated pair block
    //if(sz == 0) sz = 1;
    bd_sizes[k].alloc = p;
